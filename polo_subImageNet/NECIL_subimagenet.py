...
class polo:
    ...
    def beforeTrain(self, current_task, data_manager,class_set):
        self.model.eval()
        ############## before training ###############
        if current_task == 0:
        ...
        trainfolder = data_manager.get_dataset(self.train_transform, index=classes, train=True)
        testfolder = data_manager.get_dataset(self.test_transform, index=class_set[:self.numclass], train=False)

        self.train_loader = torch.utils.data.DataLoader(
            trainfolder, batch_size=self.args.batch_size, shuffle=True,
            drop_last=True, num_workers=8)
        self.test_loader = torch.utils.data.DataLoader(
            testfolder, batch_size=self.args.batch_size, shuffle=False,
            drop_last=False, num_workers=8)

        if current_task > 0:
            self.model.module.Incremental_learning(self.ssl * self.numclass)
        self.model.train()
        if current_task > 0:
            print()
        else:
    ...
    def train(self, current_task, old_class=0):
        ...
        if os.path.exists(exist_path):
            ...
            scheduler = StepLR(opt, step_size=self.decay_epochs, gamma=0.2)
    ...
    def _compute_loss(self, imgs, target, old_class=0):
        ...
        if self.old_model is None:
            ...
            proto_aug, proto_aug_label = self.apa(feature, self.prototype, proto_aug, proto_aug_label,old_class)
            ...
            return loss_cls + self.args.protoAug_weight*loss_protoAug + self.args.kd_weight*loss_kd, loss_cls, loss_protoAug, loss_kd

    def apa(self, feat, proto, proto_aug, proto_aug_label,old_class):
        ...
        for j in range(self.args.batch_size * self.rate):
            ...
            random_vec = np.random.normal(0, 1, 512) * self.radius[index[0]] * self.args.fg_nc/old_class
